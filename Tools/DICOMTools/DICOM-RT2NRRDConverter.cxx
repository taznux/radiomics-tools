...
//using SpatialObjectToImageFilterType = itk::SpatialObjectToImageFilter<PolygonType, ImageSliceType>;
using SpatialObjectToImageFilterType = itk::MaskedSpatialObjectToImageFilter<PolygonType, ImageSliceType>;
...
void mergeImages(ImageSliceType::Pointer &tempSlice, ImageType::Pointer &finalImage, int iRequiredSlice)
{
  ...
  if (iRequiredSlice > 0)
  {
    ...
    regionSize[0] = tempSlice->GetLargestPossibleRegion().GetSize()[0];
    regionSize[1] = tempSlice->GetLargestPossibleRegion().GetSize()[1];
    regionSize[2] = 1;

    ImageType::IndexType regionIndex;
    regionIndex[0] = 0;
    regionIndex[1] = 0;
    regionIndex[2] = iRequiredSlice;

    ImageType::RegionType region;
    region.SetSize(regionSize);
    region.SetIndex(regionIndex);

    itk::ImageRegionIterator<ImageType> finalImageIterator(finalImage, region);
    itk::ImageRegionConstIterator<ImageSliceType> tempSliceIterator(tempSlice, tempSlice->GetLargestPossibleRegion());

    while (!tempSliceIterator.IsAtEnd())
    {
      pixelValue = tempSliceIterator.Get();
      finalImageIterator.Set(finalImageIterator.Get() ^ (pixelValue != 0));
      ++finalImageIterator;
      ++tempSliceIterator;
    }
  }
...
int main(int argc, char *argv[])
{
  if (argc < 3)
  {
    ...
    std::cerr << argv[0] << "  DicomDirectory inputDICOMRTImageFile OutputFilePrefix [ContourName] [NewContourName]" << std::endl;
  ...
  size[2] = 0;
  ImageType::IndexType start = ImageType::IndexType(inputRegion.GetIndex());
  start[2] = 0;
  ...
  ImageSliceType::PointType origin;
