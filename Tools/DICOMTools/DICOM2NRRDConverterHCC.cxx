...
int main( int argc, char *argv[] )
{
    ...
    try
    {
        ...
        for ( seriesItr = seriesUID.begin(); seriesItr != seriesEnd ; seriesItr++)
        {
            ...
            //dicomIO->GetValueFromTag("0008|0031", aqusitionTime);
            ...
            dicomIO->GetValueFromTag("0008|0070", manufacturer);
            std::cout << manufacturer << std::endl;

            dicomIO->GetValueFromTag("0028|1052", rescaleIntercept);
            std::cout << rescaleIntercept << std::endl;

            typedef itk::AddImageFilter <ImageType, ImageType, ImageType> AddImageFilterType;
            AddImageFilterType::Pointer addImageFilter = AddImageFilterType::New();
            if(rescaleIntercept.length() == 0)
            {
                PixelType newRescaleIntercept = 0;
                refineString(manufacturer);
                if(manufacturer.compare("SIEMENS") == 0)
                {
                    newRescaleIntercept = -1024;
                }
                else
                {
                    newRescaleIntercept = -1000;
                }

                std::cout << "rescale " << newRescaleIntercept << std::endl;

                addImageFilter->SetInput(inputImage);
                addImageFilter->SetConstant2(newRescaleIntercept);
                addImageFilter->Update();

                inputImage = addImageFilter->GetOutput();
            }
            ...
            nrrdFilename.append(patientID + "_" + studyDate + "_" + aqusitionTime);
