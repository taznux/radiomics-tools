 ...
 *
 *=========================================================================*/
#ifndef itkScalarImageToRunLengthFeaturesFilter1_hxx
#define itkScalarImageToRunLengthFeaturesFilter1_hxx
...
ScalarImageToRunLengthFeaturesFilter1<TImage, THistogramFrequencyContainer>
...
::FullCompute()
{
  ...
  for( offsetIt = this->m_Offsets->Begin(), offsetNum = 0;
    ...
    OffsetType offset = offsetIt.Value();
    double numLevels = this->m_RunLengthMatrixGenerator->GetNumberOfBinsPerAxis();
    ...
    if (std::abs(offset[1]) == 1){
      voxelLength = std::pow(voxelLength,2) + std::pow(spacing[1],2);
      voxelLength = std::sqrt(voxelLength);
    }
    if (std::abs(offset[2]) == 1){
      voxelLength = std::pow(voxelLength,2) + std::pow(spacing[2],2);
      voxelLength = std::sqrt(voxelLength);
    }
    ...
    this->m_RunLengthMatrixGenerator->SetDistanceValueMinMax (0,dMax);
    //--------------------------------------------------------------------------
