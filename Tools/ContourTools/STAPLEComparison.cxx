...
int main(int argc, char *argv[])
{
	if (argc < 4)
	{
		...
		cerr << "Usage = " << argv[0];
		cerr << " OutputImage OutputText Label InputLabelImage1 InputLabelImage2 InputLabelImage3 ..." << endl;
		return EXIT_FAILURE;
	}
	...
	for (int i = 0; i < numInputs; i++)
	{
		...
		if (selectedLabelValue > 0)
			maskImage = GetMaskImage(ReadImageFile<LabelImageType>(inputLabelImageNames[i]), selectedLabelValue);
		else
			maskImage = GetMaskImage(ReadImageFile<LabelImageType>(inputLabelImageNames[i]));
	...
	typedef itk::STAPLEImageFilter< MaskImageType, InternalImageType > StapleFilterType;
	...
	cout << "Number of elapsed iterations = "
		<< sFilter->GetElapsedIterations() << endl;

	//  cout.precision(5);
	//  cout.setf(ios::fixed, ios::floatfield);

	cout << "File \t\t" << "\tSensitivity(p) " << "\tSpecificity(q)" << "\tJaccard Index(j)" << endl;
	cout << "-----\t\t" << "\t-------------- " << "\t--------------" << "\t--------------" << endl;
	outfile << "File " << "\tSensitivity(p) " << "\tSpecificity(q)" << "\tJaccard Index(j)" << endl;
	itk::ImageRegionIterator< InternalImageType > output(outputImage, outputImage->GetBufferedRegion());
	itk::ImageRegionIterator< MaskImageType > unionMask(unionMaskImage, unionMaskImage->GetBufferedRegion());
	...
	!output.IsAtEnd() && !unionMask.IsAtEnd();
		++output, ++unionMask)
	{
		InternalImageType::PixelType prob = output.Get();
	...
	for (int i = 0; i < numInputs; i++)
	{
		...
		itk::ImageRegionIterator< MaskImageType > individualMask(inputMaskImages[i], inputMaskImages[i]->GetBufferedRegion());
	...
	outfile << "Mean:\t" << avg_p << "\t" << avg_q << "\t" << avg_j << endl << endl;
